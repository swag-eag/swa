...
build() {
    ...
    host="$2"
    name="$3"
    pkg="swad${network}-${build_type}"
    if [[ "$host" == "native" ]]; then
        if [[ "${build_platform: -6}" == "-linux" ]]; then
            # static link for linux targets
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsStatic.swa-matrix.${pkg}"
        else
            FLAKE="${baseurl}#${pkg}"
        fi
    else
        if [[ "$host" == "aarch64-multiplatform" || "$host" == "gnu64" ]]; then
            # static link for linux targets
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsCross.${host}.pkgsStatic.swa-matrix.${pkg}"
        else
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsCross.${host}.swa-matrix.${pkg}"
        fi
    fi
    echo "building $FLAKE"
    nix build -L "$FLAKE"
    cp result "swa_${ref_name_clean:1}${network}_${name}.tar.gz"
}
