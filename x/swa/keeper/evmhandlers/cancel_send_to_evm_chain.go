...
// `event __SwaCancelSendToEvmChain(uint256 id)`
...
func (h CancelSendToEvmChainHandler) Handle(
	...
	swac20Address, found := h.swaKeeper.GetContractByDenom(ctx, denom)
	if !found {
		return fmt.Errorf("the native token %s is not associated with any contract address on swa", denom)
	}
	...
	err = h.swaKeeper.ConvertVouchersToEvmCoins(ctx, senderCosmosAddr.String(), refundAmount)
	if err != nil {
		return err
	}
	return nil
