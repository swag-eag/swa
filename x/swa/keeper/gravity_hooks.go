...
func (k Keeper) doAfterSendToCosmosEvent(ctx sdk.Context, event gravitytypes.SendToCosmosEvent) error {
	...
	// TODO: Remove after event is emitted at Gravity module https://github.com/swag-eag/gravity-bridge/pull/12
	coins := sdk.Coins{sdk.NewCoin(denom, event.Amount)}
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		swatypes.EventTypeEthereumSendToCosmosHandled,
		sdk.NewAttribute(sdk.AttributeKeyModule, gravitytypes.ModuleName),
		sdk.NewAttribute(swatypes.AttributeKeySender, event.GetEthereumSender()),
		sdk.NewAttribute(swatypes.AttributeKeyReceiver, event.GetCosmosReceiver()),
		sdk.NewAttribute(swatypes.AttributeKeyAmount, coins.String()),
		sdk.NewAttribute(gravitytypes.AttributeKeyBridgeChainID, strconv.FormatUint(
			k.gravityKeeper.GetParams(ctx).BridgeChainId, 10,
		)),
		sdk.NewAttribute(swatypes.AttributeKeyEthereumTokenContract, event.GetTokenContract()),
	...
	err = k.ConvertCoinFromNativeToSWAC21(ctx, addr, coin, enableAutoDeployment)
	if err != nil {
		return err
	}
