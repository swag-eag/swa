...
func SimulateUpdateTokenMapping(ak types.AccountKeeper, bk types.BankKeeper, k *keeper.Keeper) simtypes.Operation {
	return func(
		...
		swaAdmin := k.GetParams(ctx).SwaAdmin
		var simAccount simtypes.Account

		if r.Intn(2) > 0 {
			var found bool
			simAccount, found = findSwaAdmin(accs, swaAdmin)
			if !found {
		...
		if simAccount.Address.String() != swaAdmin && errors.Is(err, errorsmod.Wrap(sdkerrors.ErrUnauthorized, "msg sender is not authorized")) {
...
func findSwaAdmin(accs []simtypes.Account, swaAdmin string) (simtypes.Account, bool) {
	found := false
	for _, acc := range accs {
		if acc.Address.String() == swaAdmin {
			found = true
			return acc, found
		}
	}
