...
def swa_mempool(tmp_path_factory):
    path = tmp_path_factory.mktemp("swa-mempool")
...
def test_mempool(swa_mempool):
    w3: Web3 = swa_mempool.w3
    filter = w3.eth.filter("pending")
    assert filter.get_new_entries() == []

    cli = swa_mempool.cosmos_cli(0)
    # test contract
...
def test_blocked_address(swa_mempool):
    cli = swa_mempool.cosmos_cli(0)
