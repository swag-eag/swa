...
def test_ibc(ibc):
    # chainmain-1 relayer -> swa_777-1 signer2
    w3 = ibc.swa.w3
    wait_for_new_blocks(ibc.swa.cosmos_cli(), 1)
    start = w3.eth.get_block_number()
    hermes_transfer(ibc)
    denom = ibc_denom(channel, src_denom)
    dst_addr = eth_to_bech32(swa_signer2)
    ...
    swa_addr = module_address("swa")
    transfer_addr = module_address("transfer")
    seq = get_send_packet_seq(chainmain_cli)
    expected = [
        recv_packet(seq, relayer0, swa_signer2, src_amount, src_denom),
        denom_trace(denom),
        *send_from_module_to_acc(transfer_addr, swa_signer2, src_amount, denom),
        fungible(swa_signer2, relayer, src_amount, src_denom),
        *send_from_acc_to_module(swa_signer2, swa_addr, src_amount, denom),
        *send_from_module_to_acc(swa_addr, swa_signer2, dst_amount, dst_denom),
        write_ack(seq, relayer0, swa_signer2, src_amount, src_denom),
    ]
    assert len(logs) == len(expected)
    height = logs[0]["blockNumber"]
    assert_duplicate(ibc.swa.base_port(0), height)
...
def test_ibc_incentivized_transfer(ibc):
    ...
    expected = [
        ...
        *send_coins(feeibc_addr, src_relayer, src_amount, fee_denom),
        distribute_fee(src_relayer, fee),
        *send_coins(feeibc_addr, src_relayer, src_amount, fee_denom),
        distribute_fee(swa_signer2, fee),
        *send_coins(feeibc_addr, swa_signer2, src_amount, fee_denom),
        fungible(checksum_dst_adr, swa_signer2, amount, dst_denom),
        recv_packet(seq1, dst_adr, swa_signer2, amount, transfer_denom),
        *send_coins(escrow, swa_signer2, amount, dst_denom),
        fungible(swa_signer2, checksum_dst_adr, amount, transfer_denom),
        write_ack(seq1, dst_adr, swa_signer2, amount, transfer_denom),
    ]
    assert len(logs) == len(expected)
    for i, log in enumerate(logs):
        method_name, topic = get_topic_data(w3, method_map, contract_info, log)
...
def assert_transfer_source_tokens_topics(ibc, fn):
    ...
    checksum_dst_adr = to_checksum_address(bech32_to_eth(dst_adr))
    swa_addr = module_address("swa")
    swa_denom = f"swa{contract}"
    transfer_denom = f"transfer/{channel}/{swa_denom}"
    expected = [
        acknowledge_packet(seq0),
        fungible(checksum_dst_adr, ADDRS["validator"], amount, swa_denom),
        recv_packet(seq1, dst_adr, swa_signer2, amount, transfer_denom),
        *send_coins(escrow, swa_signer2, amount, swa_denom),
        fungible(swa_signer2, checksum_dst_adr, amount, transfer_denom),
        *send_coins(swa_signer2, swa_addr, amount, swa_denom),
        coin_spent(swa_addr, amount, swa_denom),
        burn(swa_addr, amount, swa_denom),
        write_ack(seq1, dst_adr, swa_signer2, amount, transfer_denom),
    ]
    assert len(logs) == len(expected)
    height = logs[0]["blockNumber"]
...
def test_swa_transfer_source_tokens(ibc):
    assert_transfer_source_tokens_topics(ibc, swa_transfer_source_tokens)


def test_swa_transfer_source_tokens_with_proxy(ibc):
    assert_transfer_source_tokens_topics(ibc, swa_transfer_source_tokens_with_proxy)
