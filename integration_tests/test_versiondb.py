...
def test_versiondb_migration(swa: Swa):
    """
    ...
    print(swa.supervisorctl("stop", "all"))
    cli0 = swa.cosmos_cli(i=0)
    cli1 = swa.cosmos_cli(i=1)

    changeset_dir = tempfile.mkdtemp(dir=swa.base_dir)
    print("dump to:", changeset_dir)
    print(cli1.changeset_dump(changeset_dir))
    snapshot_dir = tempfile.mkdtemp(dir=swa.base_dir)
    ...
    sst_dir = tempfile.mkdtemp(dir=swa.base_dir)
    ...
    print(swa.supervisorctl("start", "swa_777-1-node0", "swa_777-1-node1"))
    wait_for_port(ports.evmrpc_port(swa.base_port(0)))
    wait_for_port(ports.evmrpc_port(swa.base_port(1)))
