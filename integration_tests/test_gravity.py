...
def check_auto_deployment(cli, denom, swa_w3, recipient, amount):
    "check swac21 contract auto deployed, and the swac21 balance"
    try:
    ...
    swac21_contract = swa_w3.eth.contract(
        address=rsp["auto_contract"], abi=swa_swac21_abi()
    )
    try:
        if swac21_contract.caller.balanceOf(recipient) == amount:
            return swac21_contract
...
def get_id_from_receipt(receipt):
    ...
    target = HexBytes(
        abi.event_signature_to_log_topic("__SwaSendToEvmChainResponse(uint256)")
    )
...
def test_gravity_transfer(gravity):
    ...
    if gravity.swa.enable_auto_deployment:
        swac21_contract = None

        def local_check_auto_deployment():
            nonlocal swac21_contract
            swac21_contract = check_auto_deployment(
                cli, denom, swa_w3, recipient, amount
            )
            return swac21_contract

        wait_for_fn("send-to-swac21", local_check_auto_deployment)

        # send it back to erc20
        tx = swac21_contract.functions.send_to_evm_chain(
            ADDRS["validator"], amount, 1, 0, b""
        ).build_transaction({"from": ADDRS["community"]})
        txreceipt = send_transaction(swa_w3, tx, KEYS["community"])
        # SWAC20 emit 3 logs for send_to_evm_chain:
        # burn
        # __SwaSendToEvmChain
        # __SwaSendToEvmChainResponse
        ...
        "messages": [
            {
                ...
                "content": {
                    "@type": "/swa.TokenMappingChangeProposal",
                    "denom": denom,
                    "contract": conctract,
                    "symbol": "",
                    "decimal": 0,
...
def test_gov_token_mapping(gravity, tmp_path, is_legacy):
    ...
    - deploy corresponding contract on swa
    - add the token mapping on swa using gov module
    ...
    swac21 = deploy_contract(
        swa_w3,
        CONTRACTS["TestERC20Utility"],
    )
    print("swac21 contract", swac21.address)
    denom = f"gravity{erc20.address}"
    ...
    rsp = submit_proposal(cli, tmp_path, is_legacy, denom, swac21.address)
    assert rsp["code"] == 0, rsp["raw_log"]

    approve_proposal(gravity.swa, rsp)

    print("check the contract mapping exists now")
    rsp = cli.query_contract_by_denom(denom)
    print("contract", rsp)
    assert rsp["contract"] == swac21.address

    print("try to send token from ethereum to swa")
    txreceipt = send_to_cosmos(
    ...
    def check():
        balance = swac21.caller.balanceOf(ADDRS["community"])
        print("swac20 balance", balance)
        return balance == 10

    wait_for_fn("check balance on swa", check)

    # check duplicate end_block_events
    height = cli.block_height()
    port = ports.rpc_port(gravity.swa.base_port(0))
...
def test_direct_token_mapping(gravity):
    ...
    rsp = cli.update_token_mapping(denom, swac21.address, "", 0, from_="community")
    assert rsp["code"] != 0, "should not have the permission"

    rsp = cli.update_token_mapping(denom, swac21.address, "", 0, from_="validator")
    assert rsp["code"] == 0, rsp["raw_log"]
...
def test_gravity_cancel_transfer(gravity):
    if gravity.swa.enable_auto_deployment:
        ...
        cancel_contract = deploy_contract(
            swa_w3,
            CONTRACTS["SwaGravityCancellation"],
        )
        ...
        def local_check_auto_deployment():
            nonlocal swac21_contract
            swac21_contract = check_auto_deployment(
                cli, denom, swa_w3, community, amount
            )
            return swac21_contract
        ...
        current_block = swa_w3.eth.get_block_number()
        ...
        # Check_deduction
        balance_after_send = swac21_contract.caller.balanceOf(community)
        ...
        canceltxreceipt = send_transaction(swa_w3, canceltx, KEYS["community"])
        # Should fail because it was not called from the SWAC21 contract
        assert canceltxreceipt.status == 0, "should fail"

        canceltx = swac21_contract.functions.cancel_send_to_evm_chain(
            int.from_bytes(tx_id, "big")
        ).build_transaction({"from": community})
        canceltxreceipt = send_transaction(swa_w3, canceltx, KEYS["community"])
        ...
        def check_refund():
            v = swac21_contract.caller.balanceOf(community)
            return v == amount
...
def test_gravity_source_tokens(gravity):
    if not gravity.swa.enable_auto_deployment:
        ...
        w3 = gravity.swa.w3
        symbol = "DOG"
        contract, denom = setup_token_mapping(gravity.swa, "TestERC21Source", symbol)
        ...
        swa_receiver = "0x0000000000000000000000000000000000000001"
