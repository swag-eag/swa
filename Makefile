...
build: check-network print-ledger go.sum
	@go build -mod=readonly $(BUILD_FLAGS) -o $(BUILDDIR)/swad ./cmd/swad

install: check-network print-ledger go.sum
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/swad
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.swad/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.swad/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
